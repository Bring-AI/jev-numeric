...
| 分支表示 | 每一步问什么 | 模型返回 |
|---|---|---|
| **`interval`**（默认） | 数值落在哪个区间？ | 区间标签 |
| **[`digits`](#逐位数字解码无需-logits)** | 给定已选前缀，下一个十进制数字是什么？ | 数字 `0–9` |

**本质都是多叉决策树，十进制逐位解码是其中的十叉形式。** 在对齐的十进制网格上，两种表示可以对应完全相同的分支与叶节点，只是提示词不同。数字是 **Choice 选项**，不是词表 token；选项概率由 Jev 直接返回。
...
<sub>图中 NumericJev 使用区间解码。误差线为 95% 题目族自助法区间。LoRA 连续头是在因果分布上训练、使用不同骨干模型的迁移基线；斜线柱为输入中提供答案的对照。</sub>
...
沿多叉决策树从根节点走向叶节点，每一步调用 Jev Choice 选择分支。`interval` 和 `digits` 指定的是如何向模型描述这些分支。
...
加上 **`"method": "digits"`**，即可从选区间切换为逐位选数字：
...
```text
每一步：原始问题 + 已选数字前缀 → Choice(0, 1, …, 9)
示例路径："" → "1" → "11." → "11.7" → "11.75" → 数值 11.75
```
...
程序插入小数点并保留前导零。目前支持 `[0, 10ⁿ)` 范围、`10⁻ᵈ` 精度，`n,d` 为非负整数，共 `n+d` 次顺序 Choice 调用。超出精度的小数截断，不四舍五入；负数或其他范围可使用 `interval`。[JSON 接口及 Python 用法](docs/json-api.md#digit-by-digit-choice)。

在这种十进制网格上，每层十等分时，两种表示对应同一棵树：前缀 `0.81` 就是区间 `[0.81, 0.82)`。**区别在于如何向模型描述决策，而非树结构。** 划分相同，不代表模型的选择或概率相同。数字选项不要求一个数字对应一个 token，也不能据此断言更符合 Jev 的训练方式。两种表示都不保证预测准确或概率已校准。

[本次真实试跑：提示词修改及所有输出](artifacts/digit-decoding-place-prompt-20260924/report.md)。下方历史成绩仍对应当时的区间及逐位提示词。
...
区间与十进制逐位现已由同一接口提供。早期实验使用 12 道人工选择的算术题，含六道整数、六道可精确表示的小数；每道题两种顺序、两次重复，每种方法共 48 次评估。以下数值对应归档的旧提示词，不是当前适配器的新基准结果。
