...
**Jev 擅长结构化决策。我们利用这种能力，构建一个可指定范围与精度的数值输出接口。** 可以逐层选择区间，也可以从 `0–9` 中逐位选择十进制数字。**两种方式都只调用 Jev Choice，无需训练、连续头或底层 token logits。**
...
数字是 **Choice 选项**，不是词表 token。两种方式都直接使用 Jev 返回的选项概率。
...
在这种十进制网格上，选择前缀 `0.81` 与选择 `[0.81, 0.82)` 表示同一条分支。**区别在于如何向模型描述决策。** 数字选项不要求一个数字对应一个 token，也不能据此断言更符合 Jev 的训练方式。两种方法都不保证预测准确或概率已校准。
